...
## 후속 — 식약처 고시 대조 (선택, 플러그인 밖)

팩을 만든 뒤 `kr-claims` 스크립트로 국내 기준과 대조할 수 있다.
**플러그인에 포함되지 않은 공용 도구**이므로 경로가 있을 때만 실행한다.

```bash
python3 <repo>/pharmacist/kr-claims/scripts/check_kr_claims.py \
  --pack evidence-pack.json \
  --claims <repo>/pharmacist/kr-claims/data/kr_claims.json \
  --output kr-report.json
```

- P6과 **다른 것을 본다.** P6은 "이 숫자가 그 초록에 있는가", 이쪽은
  "이 숫자를 국내에서 써도 되는가"다. P6을 통과한 500 mg이 여기서
  상한 초과로 걸리는 것은 정상이다.
- 종료 코드 **2**면 발행 전 확인이 필요하다 (미등재 / 상한 초과 / 비교 불가).
- **미등재는 "인정되지 않음"이 아니다.** 고시형만 담긴 파일이라
  개별인정형은 별도 확인이 필요하다.
- 리포트의 `cautions`(섭취 시 주의사항)가 안전성 슬라이드의 1차 근거다.
  P5를 하지 않는 이유가 여기서 보완된다.

스크립트가 없으면 이 단계를 건너뛰되, **광고법 미검토 경고는 그대로 낸다.**
