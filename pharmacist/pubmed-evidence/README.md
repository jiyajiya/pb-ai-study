...
> 이 한정은 **문장 경계 검사가 정확한 만큼만** 참이다. 실제로 한동안 참이
> 아니었다: 경계 정규식이 `[.!?]\s+[A-Z(]`라서 **둘째 문장이 숫자로 시작하면**
> 경계로 잡히지 않았고, `"...500 mg daily. 17.36 min was..."`가 한 문장으로
> 통과해 `17.36 mg`이 만들어졌다. 지금은 숫자 시작도 경계로 본다
> (`test_verify_evidence.py`의 R1이 이 케이스를 고정한다). 경계 검사에 구멍이
> 나면 이 문단의 보증이 통째로 무효가 된다는 점은 그대로다 —
> 위 표의 "막는다" 칸은 정규식 하나에 얹혀 있다.
