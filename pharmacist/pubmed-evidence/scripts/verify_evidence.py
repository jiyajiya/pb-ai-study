...
# 둘째 문장이 숫자로 시작하면([A-Z(]에 0-9가 없던 시절) 경계로 잡히지 않았다.
# "...500 mg daily. 17.36 min was the mean reduction."이 한 문장으로 통과하면
# value=17.36 + unit=mg가 quote 전체 검사를 그대로 빠져나간다 — 이 클래스를
# 막으려고 만든 검사가 정작 그 예시를 놓치고 있었다. 숫자 시작을 포함한다.
SENTENCE_BREAK = re.compile(r"[.!?]\s+[A-Z(0-9]")
