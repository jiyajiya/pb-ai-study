...
| 3 | 값이 `null`이면(또는 공백뿐이면) 팩의 `topic`("마그네슘 × 수면") 앞부분 | `pack_topic` |
...
| `block` | 발행 전 확인 필요 — 미등재 / 상한 초과 / 비교 불가 / 용량 슬롯 없음 / 팩이 고시형 미등재로 표시(`kr_notice_name`이 공란) / 원료명을 정할 수 없음 / 실행 오류 | 2 |
